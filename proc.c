...
// Helper for readMem that checks in /proc/[pid]/maps to make sure the
// requested memory range is valid
...
/**
 * Reads the specified memory range from a process, and returns a formatted string with its contents.
 * 
 * @param pid The process in question.
 * @param offset Memory address in the VAS to start reading from.
 * @param len Amount of memory to be read, in bytes.
 * 
 * @return A formatted string with the requested memory contents in hexadecimal
 */
